...
public class FederatorScheduler implements Scheduler {

	private int taskIdCounter = 0;
	private Protos.ExecutorInfo executorInfo;

	public FederatorScheduler(Protos.ExecutorInfo executorInfo) {
	...
	public void resourceOffers(SchedulerDriver schedulerDriver, List<Protos.Offer> offers) {
		...
		for (Protos.Offer offer : offers) {
			...
			Protos.TaskInfo task = Protos.TaskInfo.newBuilder().setName("task " + taskId).setTaskId(taskId)
					.setSlaveId(offer.getSlaveId()).addResources(buildResource("cpus", 1))
					.addResources(buildResource("mem", 128)).setData(ByteString.copyFromUtf8("" + taskIdCounter))
	...
	private Protos.TaskID buildNewTaskID() {
		return Protos.TaskID.newBuilder().setValue(Integer.toString(taskIdCounter++)).build();
	}
	...
	public void statusUpdate(SchedulerDriver schedulerDriver, Protos.TaskStatus taskStatus) {
		System.out.println("Status update: " + taskStatus.getState() + " from " + taskStatus.getTaskId().getValue());
	}

	@Override
	public void frameworkMessage(SchedulerDriver schedulerDriver, Protos.ExecutorID executorID, Protos.SlaveID slaveID,
			byte[] bytes) {
		System.out.println("Received message (scheduler): " + new String(bytes) + " from " + executorID.getValue());
	}
